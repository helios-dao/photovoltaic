...
contract HlsRewardsFactory {
    ...
    event HlsRewardsCreated(address indexed rewardsToken, address indexed stakingToken, address indexed mplRewards, address owner);
    ...
    */
    ...
    /**
        ...
        @return mplRewards  Address of the instantiated HlsRewards.
    */
    function createHlsRewards(address rewardsToken, address stakingToken) external returns (address mplRewards) {
        require(msg.sender == globals.governor(), "RF:NOT_GOV");
        mplRewards               = address(new HlsRewards(rewardsToken, stakingToken, msg.sender));
        isHlsRewards[mplRewards] = true;

        emit HlsRewardsCreated(rewardsToken, stakingToken, mplRewards, msg.sender);
    }
